...
Jonathan A. Bartik
4/22/2026
High Fidelity 6DOF Rocket Simulator
%}

%% STATES
% Trajectory     % Stability                  % Fuel
x   = six(1);    p              = six(8);     m_f = six(13);
v_x = six(2);    pitch_attitude = six(9);   
v_y = six(4);    q              = six(10);
...
% Angles
rotation = specs.flight_angle;           % Initial launch angle
chi = atan2(v_z,v_y);                    % Horizontal azimuth
...
SM = (CenterP - CenterG) / D;              % Stability Margin
...
CNaf = CNalphaFins(mach, cr, ct, s, D, Nf) / Nf; % Fin normal force
cldelta = (Nf * CNaf * R_fins) / (S * D);        % Roll stability
clp = -3 * (Nf * CNaf / S) * (R_fins / D)^2;     % Roll damping

v_damp = v_safe;                                 % Prevent divide by 0
Cm_roll = cldelta * delta + clp * (p * D / (2*v_damp));        % Roll co
Cm_pitch = (cmalpha * alpha) - (cmq * (q * ell / (2*v_damp))); % Pitch co
Cm_yaw = cmalpha * beta + cmq * (r * ell / (2*v_damp));        % Yaw co 
dragCoefficient = DragCoefficient(mach, rho, v, D, ...
    Lnose, Lt, S, alpha_tot, v_x);               % Drag coefficient
DragForce = qbar * dragCoefficient * S;          % Drag force calculation

% Directional Drag Consideration
ux = v_x / v_safe;
uy = v_y / v_safe;
uz = v_z / v_safe;
...
% Principal Inertial Tensors
[Ix, Iy, Iz] = asymmetricInertia(Lt,L_fuel,m_body,m_f,m_fin,Nf,dryCG, ...
    x_fuel,x_fins,R,R_fins,CenterG,s,chord);
L = (qbar * S * D) * Cm_roll;    % Aerodynamic rolling moment
M = (qbar * S * ell) * Cm_pitch; % Aerodynamic pitching moment
N = (qbar * S * ell) * Cm_yaw;   % Aerodynamic yawing moment
...
dx(7) = p;                                        % Roll Rate
dx(8) = rail_factor * ((L - (Iz - Iy)*q*r) / Ix); % Roll Acceleration
dx(9) = q;                                        % Pitch Rate
dx(10) = rail_factor * (((M - (Ix - Iz)*p*r) / Iy) ...
    - pitch_turn);                                % Pitch Acceleration
dx(11) = r;                                       % Yaw Rate
dx(12) = rail_factor * (((N - (Iy - Ix)*p*q) / Iz) ...
    - yaw_turn);                                  % Yaw Acceleration
if m_f <= 0
    dx(13) = 0;                                   % Coasting
else
